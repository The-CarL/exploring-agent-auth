"""Pattern 6: Token Exchange (service side).

The JWT validation here is identical to pattern 5. The service validates
the signature via JWKS. The difference is what arrives: the exchanged token
has aud scoped to just this service, so the service reports method="scoped_jwt".

The magic happened at the MCP server (token exchange), not here.
"""
