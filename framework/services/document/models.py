...
service start. Documents are tagged with access_groups so the filtering
rules in app.py can show "alice sees engineering docs, dave sees admin-only
HR + board materials" through real data.
...
def init_db() -> str:
    ...
    global DB_PATH
    DB_PATH = os.path.join(tempfile.mkdtemp(), "documents.db")
