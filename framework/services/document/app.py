"""Document service FastAPI app factory.

create_app(get_identity_fn) returns a FastAPI app with the given identity
extractor bound into its routes. Filtering happens via per-document
access_groups rather than per-user ownership.
...
from framework.services.document.models import get_db, init_db


def create_app(
    get_identity_fn: Callable[..., Awaitable[Identity]],
) -> FastAPI:
    """Create a document service app with the given identity extractor."""
...
def _allowed_groups_for(identity: Identity) -> set[str] | None:
    """Returns the set of access_groups the caller is allowed to read.
    Returns None to mean 'no filtering, see everything'."""
